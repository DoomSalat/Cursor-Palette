...
- **Global cursor size** — a single slider controls the system cursor size
  (32–256 px), stored per preset.
- **Reset to Windows defaults** — a dedicated "Default" tile restores the
  system cursor scheme.
- **Undo last change** — the app keeps a snapshot of the previous cursor
  state and lets you roll back one step.
