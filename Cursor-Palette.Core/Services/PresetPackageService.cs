...
public static class PresetPackageService
{
	...
	public static (string Path, int Count) ExportBundle(IReadOnlyList<Preset> presets, string? customName = null)
	{
		...
		try
		{
			...
			{
				var presetDir = Path.Combine(stagingDir, preset.Id);
				...
				foreach (var role in CursorRoles.All)
				{
					...
					File.Copy(sourcePath, Path.Combine(presetDir, fileName), overwrite: true);
				...
				manifestPresets.Add(new ArchiveManifestPreset
				{
					...
					ScaleMode = (int)preset.ScaleMode,
					Roles = roles,
	...
	private static string GetUniqueDownloadFolderPath(string baseName)
	{
		...
		while (Directory.Exists(path))
			path = Path.Combine(PathProvider.Current.DownloadsDir, $"{baseName} ({attempt++})");
	...
	private static DetectedPackage? DetectFromExtractedDir(string extractedDir)
	{
		...
		if (manifest?.Format == BundleFormatId)
		{
			...
			{
				var filesDir = ResolvePresetFilesDir(extractedDir, preset);
			...
			var entries = archiveMarker.Presets
				...
				.Select(entry =>
				{
					...
					return new PackageEntry
					{
						...
						BaseSize = CursorConstants.DefaultBaseSize,
	...
	public static int ImportSelected(DetectedPackage package, IReadOnlyList<PackageEntry> selectedEntries,
		...
		bool ignoreIndividualSizes = false, int uniformSize = CursorConstants.DefaultBaseSize)
	{
		var imported = 0;
	...
	private static string BuildReadmeContent(string? author = null)
	{
		...
		using var stream = assetLoader.TryOpenAsset(ReadmeResourceName);
	...
	private static string ResolvePresetFilesDir(string extractedDir, ArchiveManifestPreset preset)
	{
		var legacyDir = Path.Combine(extractedDir, PresetsFolderName, preset.Id, FilesFolderName);

		return Directory.Exists(legacyDir) ? legacyDir : Path.Combine(extractedDir, preset.Folder);
	}
	...
	{
		var path = Path.Combine(PathProvider.Current.DownloadsDir, $"{baseName}{extension}");
		var attempt = 1;

		while (File.Exists(path))
			path = Path.Combine(PathProvider.Current.DownloadsDir, $"{baseName} ({attempt++}){extension}");
