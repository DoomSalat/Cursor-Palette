...
public static class GroupStore
{
	...
	{
		if (!File.Exists(PathProvider.Current.GroupsFile))
			return new();

		try
		{
			return JsonSerializer.Deserialize<List<PresetGroup>>(File.ReadAllText(PathProvider.Current.GroupsFile)) ?? new();
		}
		catch
		{
			return new();
	...
	private static void SaveAll(List<PresetGroup> groups) =>
		File.WriteAllText(PathProvider.Current.GroupsFile, JsonSerializer.Serialize(groups, JsonOptions));
