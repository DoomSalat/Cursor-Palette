...
public static class AniCursorReader
{
	...
	private static List<CursorCanvasImage> DecodeFrames(
		byte[] bytes, List<(int Offset, int Length)> iconChunks, uint declaredFrameCount)
	{
		...
		var frames = new List<CursorCanvasImage>(frameCount);
	...
	private static AnimatedCursorFrames BuildSchedule(List<CursorCanvasImage> frames, AnimationHeader header)
	{
