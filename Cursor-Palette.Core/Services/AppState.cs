...
public static class AppState
{
	...
	{
		public int DefaultBaseSize { get; set; } = CursorConstants.DefaultBaseSize;
	...
	{
		if (!File.Exists(PathProvider.Current.ActiveStateFile))
			return null;

		try
		{
			return JsonSerializer.Deserialize<ActiveState>(File.ReadAllText(PathProvider.Current.ActiveStateFile))
				?.ActivePresetId;
		}
		catch
		{
	...
	public static void SetActivePresetId(string? id) =>
		File.WriteAllText(PathProvider.Current.ActiveStateFile,
			JsonSerializer.Serialize(new ActiveState { ActivePresetId = id }));

	private static Settings LoadSettings()
	{
		if (!File.Exists(PathProvider.Current.SettingsFile))
			return new Settings();

		try
		{
			return JsonSerializer.Deserialize<Settings>(File.ReadAllText(PathProvider.Current.SettingsFile)) ?? new Settings();
		}
		catch
		{
			return new Settings();
	...
	private static void SaveSettings(Settings settings) =>
		File.WriteAllText(PathProvider.Current.SettingsFile, JsonSerializer.Serialize(settings));

	public static int GetDefaultBaseSize() =>
		Math.Clamp(LoadSettings().DefaultBaseSize, CursorConstants.DefaultBaseSize, CursorConstants.MaxBaseSize);
